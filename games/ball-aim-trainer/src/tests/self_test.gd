...
func _run() -> void:
	...
	_check(game.weapon_audio_players["rifle"].stream != game.weapon_audio_players["pistol"].stream, "步枪与手枪应使用不同枪声")
	_check(game.weapon_audio_players["rifle"].volume_db > game.weapon_audio_players["pistol"].volume_db, "步枪枪声应比手枪更响")
	...
	game._on_setting_changed("round_limit", 30.0)
	game._reset_stats()
	_check(is_equal_approx(game.time_remaining, 30.0), "一局时长应控制计时器")
	_check(is_equal_approx(game.setting_sliders["red_dot_size"].min_value, 1.0), "红点大小下限应为 1 像素")
	game._on_setting_changed("red_dot_size", 1.0)
	_check(is_equal_approx(game.red_dot.dot_diameter, 1.0), "红点应支持 1 像素")
	...
	_check(is_equal_approx(game.setting_sliders["round_limit"].min_value, 10.0), "小球上限最小应为 10")
	_check(is_equal_approx(game.setting_sliders["round_limit"].max_value, 400.0), "小球上限最大应为 400")
	game._on_setting_changed("round_limit", 10.0)
	_check(game.ball_hit_goal == 10, "小球上限设置应立即生效")
	...
	for spawn_position in game.spawn_positions:
		...
		ball_columns[spawn_position.x] = true
		balls_share_one_plane = balls_share_one_plane and is_equal_approx(spawn_position.z, -game.ball_distance)
	_check(ball_rows.size() == 4 and ball_columns.size() == 3, "黄色小球应纵向平铺为三列四行")
	_check(balls_share_one_plane, "黄色小球应位于正对玩家的同一竖直靶面")
	game.state = 2
	var countdown_before_ball_mode: float = game.time_remaining
	game._process(1.25)
	_check(game.elapsed_time >= 1.24, "固定射球模式应使用正计时")
	_check(is_equal_approx(game.time_remaining, countdown_before_ball_mode), "正计时模式不应扣减倒计时")
	...
	var ball_killed := ball_target.register_weapon_hit("body", "rifle")
	_check(ball_killed, "黄色小球应命中一枪后消失")
	game.ball_targets_hit = 9
	game._register_target_hit(ball_target, "body", ball_killed)
	_check(game.ball_targets_hit == 10 and game.state == 3, "命中小球上限后应自动结束训练")
	game._set_training_mode("humanoid", false)
	game._reset_stats()
	...
	game._on_setting_changed("round_limit", 90.0)
